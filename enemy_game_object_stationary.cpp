...
namespace game {
    ...
    float EnemyGameObjectStationary::ClampFloat(float x, float lo, float hi) {
        if (x < lo) return lo;
        if (x > hi) return hi;
        return x;
    }
    ...
    void EnemyGameObjectStationary::SetPatrolEllipse(const glm::vec3& center, float w, float h) {
    ...
    void EnemyGameObjectStationary::SetPatrolAngularSpeed(float omega_rad_per_sec) {
    ...
    void EnemyGameObjectStationary::SetDesiredInterceptTime(float seconds) {
        desired_intercept_time_ = (seconds > kMinInterceptTime) ? seconds : kMinInterceptTime;
    }

    //Course correction period
    void EnemyGameObjectStationary::SetCourseCorrectionPeriod(float seconds) {
        course_period_ = (seconds > 0.05f) ? seconds : 0.05f;
    }

    //Target set(basically always to last seen coordinates of player)
    void EnemyGameObjectStationary::SetTarget(const glm::vec3& target) {
        target_ = target;
    }

    EnemyGameObjectStationary::State EnemyGameObjectStationary::GetState() const {
        return state_;
    }
    ...
    void EnemyGameObjectStationary::PatrolStep(double dt) {
    ...
    void EnemyGameObjectStationary::InterceptStep(double dt) {
    ...
    void EnemyGameObjectStationary::RecomputeVelocityTowardTarget() {
        glm::vec3 pos = GetPosition();
