...
namespace game {
    ...
    float EnemyGameObject::ClampFloat(float x, float lo, float hi) {
        if (x < lo) return lo;
        if (x > hi) return hi;
        return x;
    }
    ...
    EnemyGameObject::EnemyGameObject(const glm::vec3& position, Geometry* geom, Shader* shader, GLuint texture, GLuint destroy_texture, int hit_points)
    ...
    void EnemyGameObject::collide(GameObject* collided_with) {
    ...
    void EnemyGameObject::SetPlayer(GameObject* player) {
        player_ = player;
    }

    GameObjectType  EnemyGameObject::GetType() {
        return GameObjectType::Enemy;
    }

    void EnemyGameObject::SetPatrolEllipse(const glm::vec3& center, float w, float h) {
    ...
    void EnemyGameObject::PatrolStep(double dt) {
    ...
    void EnemyGameObject::InterceptStep(double dt) {
    ...
    void EnemyGameObject::RecomputeVelocityTowardTarget() {
        glm::vec3 pos = GetPosition();
